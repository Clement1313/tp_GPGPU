...
__global__ void mykernel(char* buffer, int width, int height, size_t pitch)
{
  ...
  // Center the view on (-0.5, 0) and preserve aspect ratio
  float halfW = (float)width * 0.5f;
  float halfH = (float)height * 0.5f;
  float nx = ((float)x - halfW) / halfW; // normalized -1..1
  float ny = ((float)y - halfH) / halfH; // normalized -1..1
  const float scaleX = 1.5f; // distance from center to right edge (from -0.5 to 1.0)
  const float scaleY = scaleX * ((float)height / (float)width);
  float mx0 = -0.5f + nx * scaleX;
  float my0 = -ny * scaleY; // flip Y so image origin (0,0) at top-left
  ...
  if (iteration == maxIter) {
    ...
    uint8_t c = (uint8_t)(255.0f * (float)iteration / (float)maxIter);
    color = make_uchar4(c, c, c, 255);
  }
